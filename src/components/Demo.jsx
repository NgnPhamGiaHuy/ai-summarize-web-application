...
const Demo = () => {
    ...
    const handleRemove = (removeUrl) => {
        const updateArticles = allArticles.filter((article) => article.url !== removeUrl);
                    ...
                    {allArticles.map((item, index) => (
                        <div key={`link-${index}`} onClick={() => setArticle(item)} className="link_card">
                            <div className="copy_btn" onClick={() => handleCopy(item.url)}>
                                <img src={copied === item.url ? tick : copy} alt={copied === item.url ? "tick_icon" : "copy_icon"}
                                     className="w-[40%] h-[40%] object-contain"/>
                            </div>
                            <p className="flex-1 font-satoshi text-blue-700 font-medium text-sm truncate">{item.url}</p>
                            <div className="copy_btn" onClick={() => handleRemove(item.url)}>
                                <img src={remove} alt="remove_icon"
                                     className="w-[40%] h-[40%] object-contain"/>
                            </div>
                        </div>
                    ))}
